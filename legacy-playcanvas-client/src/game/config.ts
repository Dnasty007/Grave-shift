...
export type MapImportVisualConfig = {
   ...
   * Defaults in `Map.loadImportedVisual` to `hytopiaContent.HYTOPIA.blockTextures` when omitted.
   */
...
/**
 * **Map 1 (castle preset):** HYTOPIA World Editor export `Topia_Map_V1` (~3.4k blocks).
 * Textures live beside the JSON under `public/maps/topia-map-v1/blocks/`.
 * Mystery box + Pack-a-Punch sit on opposite corners of the island; teddies use {@link GAME_CONFIG.teddyBearEasterEgg}.
 */
const CASTLE_IMPORT: MapImportVisualConfig = {
  enabled: true,
  replacesArena: true,
  useProceduralTestGround: false,
  jsonVoxelMapUrl: "/maps/topia-map-v1/map.json",
  jsonVoxelTextureBaseUrl: "/maps/topia-map-v1/blocks",
  /** Small export — mesh full map (no crop). Large Hytopia worlds must set a tight {@link MapJsonVoxelCrop}. */
  jsonVoxelCrop: undefined,
  /** Legacy Mineways path (unused when {@link jsonVoxelMapUrl} is set). */
  glbUrl: "/maps/world.glb",
  scale: [1, 1, 1],
  /** Island already sits mostly on −X; keep origin so editor coords match debug. */
  position: [0, 0, 0],
  eulerDegrees: [0, 0, 0],
  cameraFarClip: 420,
  /** Center-ish over the island; Y is a hint — feet snap to voxel tops on start. */
  playerSpawn: { x: -22, y: 14, z: -4 },
  playerSpawnYawDegrees: 35,
  openWorldMoveMultiplier: 1.0,
  spawnRing: { min: 10, max: 24 },
...
export const GAME_CONFIG = {
  ...
  packAPunch: {
    ...
    modelScale: 1.15,
    interactRadius: 2.85,
    /** Map 1: +Z corner of the Topia island (mystery chest is on the −X/−Z side). */
    position: { x: -3, y: 0, z: 19 },
    rotationYDegrees: -42
  },
  /**
   * COD-style mystery chest (procedural mesh). Map 1: −X / −Z corner of the island; feet Y from terrain snap.
   */
  mysteryBox: {
    position: { x: -46, y: 0, z: -22 }
  },
  player: {
    maxHealth: 100,
    moveSpeed: 6.2,
  ...
  /**
   * Boss glTF/GLB (PlayCanvas `container`). Previous ship: `dragonice.gltf` — swap `gltfUrl` to revert.
   * `Dragon_V3.blend` → Blender glTF export as `dragon_v3.glb`.
   */
  bossContent: {
    gltfUrl: "/models/boss/dragon_v3.glb",
  ...
  teddyBearEasterEgg: {
    ...
    /**
     * When **exactly three** entries are set, teddy XZ uses Hytopia block centers `(bx + 0.5, bz + 0.5)`
     * and `by` lifts the terrain ray. Otherwise `spawnPositions` is used.
     */
    spawnHytopiaBlocks: [] as ReadonlyArray<{ bx: number; by: number; bz: number }>,
