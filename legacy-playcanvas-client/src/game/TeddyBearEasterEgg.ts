...
export class TeddyBearEasterEgg {
  ...
  /** Hytopia block centers vs manual world coords (see `GAME_CONFIG.teddyBearEasterEgg`). */
  private resolveSpawnSpecs(): Array<{ x: number; y: number; z: number }> {
    const cells = this.cfg.spawnHytopiaBlocks;
    if (cells.length === 3) {
      return cells.map((b) => ({
        x: b.bx + 0.5,
        z: b.bz + 0.5,
    ...
    return [...this.cfg.spawnPositions];
  }
  ...
  async loadAndSpawn(): Promise<void> {
    ...
    const specs = this.resolveSpawnSpecs();
